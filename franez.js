...
fetch('/version.json?'+Date.now()).then(r=>r.json()).then(v=>{APP_VERSION=v;const el=document.getElementById('ver-num');if(el)el.textContent='v'+v.version;}).catch(()=>{const el=document.getElementById('ver-num');if(el)el.textContent='v?';});
