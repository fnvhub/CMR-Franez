...
function renderAcerca(){fetch('/version.json?'+Date.now()).then(r=>r.json()).then(v=>{var el=document.getElementById('about-version');if(el)el.textContent='v'+v.version;var ch=document.getElementById('about-changelog');if(!ch)return;if(v.historial&&v.historial.length){ch.innerHTML=v.historial.map(function(h){return '<div class="changelog-item"><span class="changelog-version">v'+h.version+'</span><span class="changelog-desc">'+h.cambios.join(' ')+'</span></div>';}).join('');}else{ch.innerHTML='<div class="changelog-item"><span class="changelog-desc">Sin historial</span></div>';}}).catch(function(){});}
